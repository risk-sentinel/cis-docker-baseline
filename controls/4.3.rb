...
control 'C-4.3' do
  ...
  if uri.to_s.empty?
    describe 'Requires manual review and attestation' do
      skip "Requires manual review and attestation provided for this control (\"unnecessary packages\" is consumer-policy-specific (per-image baseline) — operators attest from their image-build manifest / Dockerfile review) [Lift: set boundary_docs_base / c_4_3_attestation_uri, or `saf attest apply`.]"
    end
  else
    doc = document_attestation(uri, max_age_days: input('attestation_max_age_days', value: 365))
    describe "Boundary policy attestation (C-4.3) (#{uri})" do
