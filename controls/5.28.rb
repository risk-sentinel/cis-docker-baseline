...
control 'C-5.28' do
  ...
  uri = input('c_5_28_attestation_uri', value: '')
  uri = attestation_uri(:boundary, 'C-5.28') if uri.to_s.empty?
  if uri.to_s.empty?
    describe 'Requires manual review and attestation' do
      skip "Requires manual review and attestation provided for this control (\"latest version of image\" is enforced at build/deploy time by the consumer's image-pinning policy; operators attest from their CI/CD image-tag manifest) [Lift: set boundary_docs_base / c_5_28_attestation_uri, or `saf attest apply`.]"
    end
  else
    doc = document_attestation(uri, max_age_days: input('attestation_max_age_days', value: 365))
    describe "Boundary policy attestation (C-5.28) (#{uri})" do
