...
control 'C-5.8' do
  ...
  describe 'Requires manual review and attestation' do
    skip "Requires manual review and attestation provided for this control (privileged-port mapping (host port < 1024) is configured at the orchestration layer — task definition portMappings on ECS, container ports on Kubernetes, `--publish` flags on raw docker run — and is not observable from inside the container itself. Operator attests from the port-mapping inventory in the orchestrator's spec.)"
  end
end
