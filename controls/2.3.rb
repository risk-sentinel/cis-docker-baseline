...
control 'C-2.3' do
  ...
  if input('docker_target_mode') == 'container_only'
    describe 'AWS shared-responsibility inheritance' do
      it 'is satisfied by AWS-managed controls — AWS controls dockerd-equivalent logging on Fargate hosts; consumers consume container logs via the awslogs driver configured per-task in the ECS task definition, not at the daemon level (evidence: AWS SOC 2 Type II, AWS FedRAMP Moderate, AWS FedRAMP High, AWS ISO 27001; AWS Artifact: https://console.aws.amazon.com/artifact/)' do
